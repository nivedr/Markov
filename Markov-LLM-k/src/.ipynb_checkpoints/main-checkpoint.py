...
from optim.utils import get_batch, estimate_CE
import distributed
...
def get_args():
    ...
    parser.add_argument('--transition', default='switching', choices=['random', 'switching', 'interpolation_random', 'interpolation_uniform'])
    parser.add_argument('--interpolation', default=0.1, type=float)
...
def main(args): 
    ...
    if args.transition == 'random':
        ...
        args.P = P
    elif args.transition == 'interpolation_random':
        p = args.p
        q = args.q
        ...
        P = torch.mul(P,1-delta) + torch.mul(Q,delta)
        args.P = P
        print(P)
        print(delta)
    elif args.transition == 'interpolation_uniform':
        p = args.p
        q = args.q
        ...
        # transition 2 (uniform): Q
        Q = torch.mul(torch.ones([2**order,1]),0.5)
        Q = torch.cat((Q,1-Q),dim=1)
    ...
    CE = estimate_CE(P=P, order=order, seq_length=100000, batch_size=20, generator=cpu_generator, extra_args=args, device='cpu')
    print(f'Estimated CE is: {CE}')
    ...
    stats = train(model, CE, tokenizer_model, opt, P, order, scheduler, args.iterations, args.acc_steps, args.batch_size, args.sequence_length, int(np.mean(tok_len)), generator,
