...
def estimate_CE(P, order, seq_length, batch_size, generator, extra_args, device='cpu'):
    data = torch.zeros(batch_size, seq_length+1, device=device).long()
    c_prob = torch.zeros(batch_size, seq_length-order, device=device)
    # if extra_args.initial == 'steady':
    #     alpha = q / (p+q)
    if extra_args.initial == 'uniform':
        alpha = 0.5
    ...
    # Generate first k bits
    for k in range(order):
        data[:,k] = torch.bernoulli(alpha*torch.ones((batch_size,), device=device), generator=generator).long()
    for i in range(order, seq_length):
        data[:,i] = get_next_symbols(P, data[:,i-order:i], device).long()
        c_prob[:,i-order] = conditional_prob(P, data[:,i], data[:,i-order:i], device)
    log_c_prob = torch.log(c_prob)
    return -torch.mean(log_c_prob)
...
def conditional_prob(P, symbol, data, device='cpu'):
    ...
    M = P.to(device)[idx.to(int)]
    symbol_stack = torch.stack((symbol,symbol), dim=1)
    return torch.gather(M, 1, symbol_stack)[:,0]    
