...
def train_base(model, CE, tokenizer, opt, P, order, scheduler, iterations, acc_steps, batch_size, sequence_length, model_width, generator, eval_freq, ckpt_path, distributed_backend, extra_args):
    ...
    while itr < iterations:
        ...
        if itr == iterations or itr % 10 == 0: # from here it's only evaluation code, all the training is above
            if distributed_backend.is_master_process():
                ...
                val_acc, val_loss, val_perplexity = eval(model, tokenizer, P, order, sequence_length, model_width, batch_size,
                ...
                # if itr == iterations:
                    ...
                    #     print(param.data)
                #     _, _, _, prob_vec = eval_probs(model, tokenizer, p, q, order, sequence_length, model_width, generator, extra_args,
    ...
    with open('val-loss-dump.pickle', 'wb') as handle:
        pickle.dump([CE, val_loss_list], handle, protocol=pickle.HIGHEST_PROTOCOL)
